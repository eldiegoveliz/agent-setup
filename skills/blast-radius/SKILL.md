...
Trace how the change affects its callers, shared data, and dependent behavior.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. When history matters, inspect the relevant commits with `git log` and `git show`, and read associated review discussion through an available tool. If history or discussion is unavailable, state that gap.
...
6. If the user requests alternative reviews, use `arena` to compare two independent assessments. Follow its model and delegation settings. Keep the review read-only and verify the evidence behind each finding before combining results.
